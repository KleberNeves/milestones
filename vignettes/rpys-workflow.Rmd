...
The first step is to search for the terms on Web of Science. If you are conducting your own search, just export it as indicated in [this vignette](!https://www.bibliometrix.org/vignettes/Introduction_to_bibliometrix.html) from package *bibliometrix* (which is used under the hood to read the data). An example of such data is available in this package, so you can just import it as indicated below.
...
Alternatively, the save_papers function accepts a list of results to save in a single list of papers. Example (not run):
